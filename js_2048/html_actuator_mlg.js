...
var dogeSayings = [ "So unfair!", "True perfection has to be imperfect", "Screw you guys I'm going home", "wat", 'Dang Son Where Did You Find This', 'Smoke Weed Everyday', '9 + 10 = 21', 'leik dis if u cri evritiem', 'MLG PRO', 'ILLUMINATI CONFIRMED', 'WOMBO COMBO HAPPY FACE', "RKO'd", 'OH BABY A QUADRUPLE', 'OH OH MY GOD', 'MOM GET THE CAMERA' ,'THATS RIGHT GET NOSCOPED',
'wow', 'My Name Is Jeff', 'Do It!', 'Why You Always Lying?', 'fresh a-voca-do', 'Breakfast!', "I'm in love with the coco", 'That was legitness', 'Not to be racist or anything', 'WHAT ARE THOSE!!??' ,'9spooky25me', "get rekt m9", 'u wot m9', '360 quickscope', 'MLG', "yolo", 'swag 3 da maxx', '421 blaze it', 'such good', 'so amaze', 'many points', 'very unstoppable', 'great jorb', 'such playing', 'very good', 'points', 'very gaming', 'such player', 'concern' ,'bewildered',
'many game', 'so good', 'very scores', 'so scoring', 'so hot right now', 'such playing', 'such matching', 'so matched', 'very matched', 'very neat', "JOHN CEENA", "You can't see me", "Deez nuts", 'such natural', 'So fair!']
...
HTMLActuator.prototype.addTile = function (tile) {
  ...
  if (tile.value > Infinity) classes.push("tile-super");
...
HTMLActuator.prototype.updateScore = function (score) {
  ...
  function _randomScore() {
    ...
    // Seven digit number
    else if (random > 0.085 && random < 0.09) {
      wang = wang + Math.floor(Math.random() * 10000000);
    }
    // Eight digit number
    else if (random > 0.085 && random < 0.042) {
      wang = wang + Math.floor(Math.random() * 100000000);
    }
    // Nine digit number
    else if (random > 0.085 && random < 0.1) {
      wang = wang + Math.floor(Math.random() * 1000000000);
    }
    return sign + wang;
  }
  ...
  if (!this.boardContainer.classList.contains('rotated') && Math.random() > 0.95 && score > 3100) {
    ...
    //this.announce("It’s time for Mega Rekt. Let the board rotate!");
    this.announce("Let the board rotate!");
    var self = this
    setTimeout(function () {
...
HTMLActuator.prototype.scoreTweetButton = function () {
  ...
  tweet.setAttribute("data-counturl", "http://thereal4096.github.io/4294967296/mlg.html");
  tweet.textContent = "Tweet";

  var text = "I am the MLG pro! #2048MLG";
